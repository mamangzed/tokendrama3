...
export default async function handler(req, res) {
  ...
  // Helper local function agar bisa akses variabel `headers`
  ...
  // 1. Coba Request ke Server Utama
  try {
    const payload = {};
  ...
  // 2. Jika Gagal, Coba Request ke Fallback
  if (!finalToken) {
    try {
        const gettokenvercel = await axios.get('https://token-drama1.vercel.app/api');
  ...
  // 3. Return JSON Response ke User
